...
cd $COVID_REPO
git pull
docker run wckdouglas/md_covid19 get --date $TODAY > data/${TODAY}.tsv
echo Fetched $TODAY
...
# generate dashboard
docker run -v "$(pwd):/data" wckdouglas/md_covid19 update -o /data/dashboard.html --datadir /data/data
cd $WEB_REPO
git pull
